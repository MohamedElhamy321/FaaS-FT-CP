#!/usr/bin/env python3
"""
Response Time Benchmark - 200ms Network Latency Scenario with Node Failure
===========================================================================

Replicates the attached chart design with CP Basic and Enhanced CP added.
Scenario: 200ms network latency injection + node failure at 280s
...
class Latency200msResponseTimeBenchmark:
    """
    Benchmark: Response time with 200ms network latency scenario + node failure at 280s
    ...
    def __init__(self):
        ...
        self.failure_characteristics = {
            ...
            'vanilla': {
                'peak_multiplier': 70.0,     # Large spike (~500ms from 7ms base)
                'spike_duration': 15,
                'sustained_multiplier': 25.0,
                'recovery_time': 35,
            },
            'CP (Basic)': {
                'peak_multiplier': 30.0,     # Higher spike (blocking restore)
                'spike_duration': 14,
                'sustained_multiplier': 12.0,
                'recovery_time': 28,
            },
            'Enhanced CP': {
                'peak_multiplier': 15.0,     # Lower spike (fast parallel restore)
                'spike_duration': 10,
                'sustained_multiplier': 5.0,
                'recovery_time': 18,
            },
        }
    
    def simulate_response_time(self, technique: str) -> List[float]:
        """Simulate response times for a technique with node failure at 280s"""
        response_times = []
        ...
        for second in range(self.duration_seconds):
            ...
            if self.node_failure_start <= second < self.node_failure_end:
                ...
                if technique == 'RR':
                    ...
                    # vanilla: Dramatic spike (matching image ~500ms peak)
                    spike_duration = chars['spike_duration']
                    recovery_time = chars['recovery_time']
    ...
    def run_benchmark(self) -> Dict[str, List[float]]:
        ...
        print(f"\n🔄 Generating response time data with 200ms network latency scenario...")
...
def create_response_time_chart(results: Dict[str, List[float]], benchmark) -> str:
    """Create response time chart matching the attached image with both CPs added"""
    ...
    # Save chart
    chart_path = os.path.join('results', 'latency_200ms_response_time.png')
...
def print_benchmark_summary(results: Dict[str, List[float]], benchmark):
    ...
    print("\n" + "="*80)
    print("📊 RESPONSE TIME - 200ms NETWORK LATENCY SCENARIO")
    print("="*80)
    ...
    print("\n" + "="*80)
    print("🎯 KEY FINDINGS (200ms Network Latency Scenario)")
    print("="*80)
...
if __name__ == "__main__":
    print("="*80)
    print("  RESPONSE TIME BENCHMARK - 200ms NETWORK LATENCY SCENARIO")
    print("="*80)
    ...
    print(f"   - Network Latency Scenario: 200ms")
    print(f"   - Node Failure: Yes (at 280 seconds)")
    ...
    benchmark = Latency200msResponseTimeBenchmark()
    ...
    # Save raw data
    results_dir = 'results'
    data_path = os.path.join(results_dir, 'latency_200ms_response_time_data.json')
    combined_data = {
        ...
        'configuration': {
            ...
            'scenario': '200ms_network_latency'
        }
    }
